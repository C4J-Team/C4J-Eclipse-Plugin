...
import de.vksi.c4j.eclipse.plugin.quickassist.C4JContract;
...
public class ContractWizardPage extends NewTypeWizardPage {
	...
	protected void createTypeMembers(IType type, ImportsManager imports, IProgressMonitor monitor)
		...
		C4JContract contract = new C4JContract(type);
		...
		if(isCreateAllMethodStubs()){
			...
			contract.addMethodStubs(methodsContainedInTarget.toArray(new IMethodBinding[methodsContainedInTarget.size()]));
	...
	private List<IMethodBinding> getOnlyMethodsContainedInTarget(IType type) throws JavaModelException {
		String superTypeName = type.getSuperclassName() != null ? type.getSuperclassName() : type.getSuperInterfaceNames()[0]; 
